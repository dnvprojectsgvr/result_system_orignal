...
$result = mysql_query("SELECT ud.`security_ques`,ud.`security_ans`,f.`faculty_id`,f.`f_name`,s.`m_name`,s.`l_name`,s.`full_name`,s.`father_name`,s.`gender`,s.`mob_no`,s.`email_id`,s.`dob`  from users_details ud,faculty_details s where ud.user_id='$uidl' and s.faculty_id='$uidl'")or die("error in query");
